<template>
	<div>
		<div>
			<ul class="cityWord-con" ref="aside">
				<li class="cityWord-A" v-for="(item,key) in domesticInfo.cities" :key="item.id" @touchstart="handleLetterTop">{{key}}</li>
			</ul>
		</div>
		<div class="listCity-con">
			<div class="position">您的位置</div>
			<div class="positionCity">
				<span class="hotCity-item">{{domesticInfo.position}}</span>
			</div>
			<div class="positionCity-con">
				<div class="positionCity-title">热门城市</div>
				<div class="positionCity-item" v-for="item in domesticInfo.hotCities">
					{{item}}
				</div>
			</div>
			<ul class="cityWord-con-con">
				<li class="cityLetter" v-for="(item,key) in domesticInfo.cities">
					<span class="letterleft" :ref="key">{{key}}</span>
					<ul>
						<li class="cityWordS" v-for="elem in item">{{elem}}</li>
					</ul>
				</li>
			</ul>
		</div>
...
<script>
	...
	export default {
		...
		data() {
			...
			updated() {
				var j = 0;
				for(var i in this.domesticInfo.cities) {
...
<style scoped>
	...
	.positionCity-con {
		...
		background: #fff;
		border-bottom: .01rem solid #c9cccd;
	}
